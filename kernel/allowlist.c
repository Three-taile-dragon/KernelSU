...
void do_persistent_allow_list(struct work_struct *work)
{
	...
	if (ksu_kernel_write(fp, &magic, sizeof(magic), &off) != sizeof(magic)) {
		pr_err("save_allow_list write magic failed.\n");
		goto exit;
	}

	if (ksu_kernel_write(fp, &version, sizeof(version), &off) !=
	    sizeof(version)) {
	...
	list_for_each (pos, &allow_list) {
		...
		pr_info("save allow list uid :%d, allow: %d\n", p->uid,
			p->allow);
		ksu_kernel_write(fp, &p->uid, sizeof(p->uid), &off);
		ksu_kernel_write(fp, &p->allow, sizeof(p->allow), &off);
	}
...
void do_load_allow_list(struct work_struct *work)
{
	...
	// verify magic
	if (ksu_kernel_read(fp, &magic, sizeof(magic), &off) != sizeof(magic) ||
	    magic != FILE_MAGIC) {
		pr_err("allowlist file invalid: %d!\n", magic);
		goto exit;
	}

	if (ksu_kernel_read(fp, &version, sizeof(version), &off) !=
	    sizeof(version)) {
	...
	while (true) {
		u32 uid;
		bool allow = false;
		ret = ksu_kernel_read(fp, &uid, sizeof(uid), &off);
		if (ret <= 0) {
			pr_info("load_allow_list read err: %d\n", ret);
			break;
		}
		ret = ksu_kernel_read(fp, &allow, sizeof(allow), &off);
