				  ...
				  unsigned expected_hash)
{
	...
	for (int i = 0;; ++i) {
		...
		ksu_kernel_read(fp, &n, 2, &pos);
		if (n == i) {
			pos -= 22;
			ksu_kernel_read(fp, &size4, 4, &pos);
	...
	pos += 12;
	// offset
	ksu_kernel_read(fp, &size4, 0x4, &pos);
	pos = size4 - 0x18;

	ksu_kernel_read(fp, &size8, 0x8, &pos);
	ksu_kernel_read(fp, buffer, 0x10, &pos);
	if (strcmp((char *)buffer, "APK Sig Block 42")) {
		goto clean;
	}

	pos = size4 - (size8 + 0x8);
	ksu_kernel_read(fp, &size_of_block, 0x8, &pos);
	...
	for (;;) {
		uint32_t id;
		uint32_t offset;
		ksu_kernel_read(fp, &size8, 0x8, &pos); // sequence length
		if (size8 == size_of_block) {
			break;
		}
		ksu_kernel_read(fp, &id, 0x4, &pos); // id
		offset = 4;
		pr_info("id: 0x%08x\n", id);
		if ((id ^ 0xdeadbeefu) == 0xafa439f5u ||
			(id ^ 0xdeadbeefu) == 0x2efed62f) {
			ksu_kernel_read(fp, &size4, 0x4,
					&pos); // signer-sequence length
			ksu_kernel_read(fp, &size4, 0x4, &pos); // signer length
			ksu_kernel_read(fp, &size4, 0x4,
